...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(
      "Failed to create passkey login attempt audit log:",
      auditLogError,
    );
  ...
  try {
    ...
    if (!passkey) {
      ...
      try {
        ...
        console.error(
          "Failed to create passkey not found audit log:",
          auditLogError,
        );
      }
    ...
    if (attemptLogId) {
      try {
        ...
        console.error(
          "Failed to update passkey login attempt audit log:",
          updateError,
        );
      }
    }
    ...
    if (!verification.verified) {
      ...
      try {
        ...
        console.error(
          "Failed to create passkey verification failure audit log:",
          auditLogError,
        );
      }
    ...
    if (user.twoFactorMode === "PASSKEY_PLUS_PASSWORD") {
      ...
      try {
        ...
        console.error(
          "Failed to create passkey first factor success audit log:",
          auditLogError,
        );
      }
    ...
    try {
      await createAuditLog("LOGIN_PASSKEY_SUCCESS", user.id, user.id, {
        email: user.email,
        role: user.role,
        passkeyId: passkey.id,
        passkeyName: passkey.name,
      ...
      console.error(
        "Failed to create passkey login success audit log:",
        auditLogError,
      );
    }
