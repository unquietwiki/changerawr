...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (email.endsWith("@changerawr.sys")) {
      try {
        await createAuditLog("LOGIN_FAILURE", null, null, {
        ...
        console.error(
          "Failed to create system account block audit log:",
          auditLogError,
        );
      }
      ...
      await bcrypt.compare(
        password,
        "$2a$12$dummy.hash.to.prevent.timing.attacks.with.enough.length.to.be.realistic",
      );
    ...
    if (!user) {
      ...
      try {
        ...
        console.error(
          "Failed to create login failure audit log:",
          auditLogError,
        );
      }
    ...
    if (!isValidPassword) {
      ...
      try {
        await createAuditLog("LOGIN_FAILURE", user.id, user.id, {
    ...
    if (!bypassBreachWarning) {
      ...
      if (breachResult.isBreached) {
        ...
        try {
          await createAuditLog("PASSWORD_BREACH_DETECTED", user.id, user.id, {
            email,
          ...
          console.error(
            "Failed to create breach detection audit log:",
            auditLogError,
          );
        }
    ...
    if (attemptLogId) {
      try {
        ...
        console.error(
          "Failed to update login attempt audit log with user ID:",
          updateError,
        );
    ...
    try {
      await createAuditLog("LOGIN_SUCCESS", user.id, user.id, {
        email: user.email,
        role: user.role,
        ipAddress,
        userAgent,
    ...
    // This automatically handles localhost, custom domains, internal networks, etc.
    ...
    try {
      ...
      console.error(
        "Failed to create login completion error audit log:",
        auditLogError,
      );
    }
...
export async function GET(request: Request) {
  ...
  try {
    await createAuditLog("LOGIN_INVALID_METHOD", null, null, {
      method: "GET",
      ipAddress,
      userAgent,
      timestamp: new Date().toISOString(),
