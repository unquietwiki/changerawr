...
export async function GET(
  ...
  try {
    ...
    if (!code) {
      return NextResponse.redirect(
        `${baseUrl}/login?error=Missing+authorization+code`,
      );
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        `${baseUrl}/login?error=${encodeURIComponent("Database lookup failed")}`,
      );
    }
    ...
    if (!provider) {
      ...
      return NextResponse.redirect(
        `${baseUrl}/login?error=${encodeURIComponent(`Provider unavailable: ${providerName}`)}`,
      );
    }
    ...
    return NextResponse.redirect(
      `${baseUrl}/login?error=${encodeURIComponent("Unexpected authentication error")}`,
    );
  }
}
